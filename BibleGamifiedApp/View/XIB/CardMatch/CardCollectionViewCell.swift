...
//            frontImageView.alpha = 0
 ...
 //           return          // Exit the method if the statement is true
    ...
    func flip() {
        UIView.transition(from: backImageView, to: frontImageView, duration: 1, options: [.transitionFlipFromLeft, .showHideTransitionViews], completion: nil)
    }

    func flipBack() {

        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.8) {
            UIView.transition(from: self.frontImageView, to: self.backImageView, duration: 1, options: [.transitionFlipFromRight, .showHideTransitionViews], completion: nil)
        }
    }
