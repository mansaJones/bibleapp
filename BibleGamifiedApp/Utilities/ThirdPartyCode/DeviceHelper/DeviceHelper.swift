...
extension Device {
    ...
    public var isXSeriesDevice: Bool {
        return (isOneOf(Device.allDevicesWithSensorHousing)
            || isOneOf(Device.allSimulatorDevicesWithSensorHousing))
    }
    ...
    public var safeAreaButtom: CGFloat {
        let window = UIApplication.shared.keyWindow
    ...
    public var is169Ratio: Bool {

        let ratio = Device.current.screenRatio

        if ratio.height == 16 && ratio.width == 9 {
            return true
        } else {
            return false
        }
