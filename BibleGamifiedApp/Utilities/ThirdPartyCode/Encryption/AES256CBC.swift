...
// Encrypt
// let encryptedEmail = AES256CBC.encryptString("hello", password: Encryption.password)

// Decrypt
// let decryptedEmail = AES256CBC.decryptString("cypher text", password: Encryption.password)
    ...
    class func encryptString(_ str: String, password: String) -> String? {
        if !str.isEmpty && password.count == 32 {
            ...
            guard let encryptedString = try? aesEncrypt(str, key: key, iv: iv) else {
                Logger.log("an error occured while encrypting")
                return nil
            }
    ...
    class func decryptString(_ str: String, password: String) -> String? {
        if str.count > 16 && password.count == 32 {
            ...
            guard let decryptedString = try? aesDecrypt(encryptedString, key: password, iv: iv) else {
                Logger.log("an error occured while decrypting")
                return nil
            }
...
final private class AESCipher {
    ...
    private lazy var expandedKey: [Array[UInt32]> = self.expandKey(self.key, variant: self.variant)
    private lazy var expandedKeyInv: [Array[UInt32]> = self.expandKeyInv(self.key, variant: self.variant)
    ...
    fileprivate func expandKeyInv(_ key: Key, variant: Variant) -> [Array[UInt32]> {
        let rounds = variant.Nr
        var rk2: [Array[UInt32]> = expandKey(key, variant: variant)
    ...
    fileprivate func expandKey(_ key: Key, variant: Variant) -> [Array[UInt32]> {

        func convertExpandedKey(_ expanded: [UInt8]) -> [Array[UInt32]> {
            var arr = [UInt32]()
...
extension AESCipher {
    ...
    fileprivate func calculateSBox() -> (sBox: [UInt32], invSBox: [UInt32]) {
...
fileprivate extension UInt32 {
    ...
    func bytes(totalBytes: Int = MemoryLayout<UInt32>.size) -> [UInt8] {
        return arrayOfBytes(value: self, length: totalBytes)
    }
}

private func toUInt32Array(slice: ArraySlice<UInt8>) -> [UInt32] {
    var result = [UInt32]()
