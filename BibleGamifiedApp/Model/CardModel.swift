...
class CardModel {
    ...
    func getCards() -> [Card] {
        ...
        while generatedNumbersArray.count < 3 {

            // Get a random number
            let randomNumber = arc4random_uniform(7) + 1      // gives us randoms from 0 to 12
            ...
            if generatedNumbersArray.contains(Int(randomNumber)) == false {
                ...
                let cardOne = Card()
                cardOne.imageName = "ic_animal_\(randomNumber)"
                generatedCardsArray.append(cardOne)

                // Create the second card object
                let cardTwo = Card()
                cardTwo.imageName = "ic_animal_\(randomNumber)"
